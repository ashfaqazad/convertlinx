...
export default function ImageResizer() {
  ...
  const loadFiles = (fileList) => {
    ...
    Promise.all(promises).then(items => {
      setFiles(prev => {
        ...
        if (!prev.length && items.length) { setPxW(String(items[0].w)); setPxH(String(items[0].h)); }
        return combined;
      });
  ...
  const onDrop = useCallback((e) => { e.preventDefault(); setDragOver(false); loadFiles(e.dataTransfer.files); }, []);
  const removeFile  = (idx) => setFiles(prev => prev.filter((_, i) => i !== idx));
  const handleReset = () => { setFiles([]); setPxW(''); setPxH(''); setPct(100); setActivePreset(null); };

  /* ── Pixels lock ratio ── */
  const onPxW = (val) => {
    setPxW(val);
    if (locked && firstFile && val) { const r = firstFile.h / firstFile.w; setPxH(String(Math.round(parseInt(val) * r))); }
  };
  const onPxH = (val) => {
    setPxH(val);
    if (locked && firstFile && val) { const r = firstFile.w / firstFile.h; setPxW(String(Math.round(parseInt(val) * r))); }
  };

  /* ── get target dims ── */
  const getTargetDims = (item) => {
    if (activeTab === 'Pixels')     return { w: parseInt(pxW) || item.w, h: parseInt(pxH) || item.h };
    if (activeTab === 'Percentage') return { w: Math.round(item.w * pct / 100), h: Math.round(item.h * pct / 100) };
    if (activeTab === 'Presets' && activePreset) return { w: activePreset.w, h: activePreset.h };
  ...
  const downloadAll = async () => {
    ...
    try {
      ...
      for (const item of done) {
        ...
        zip.file(`resized-${item.file.name.replace(/\.[^.]+$/, '')}.${ext}`, blob);
      }
      ...
      for (const item of done) { downloadOne(item); await new Promise(r => setTimeout(r, 150)); }
    }
    setZipping(false);
  };
  ...
  const faqs = [
    { q: 'How do I resize an image without losing quality?',
      a: 'Shrinking an image always maintains quality since you\'re removing pixels. Enlarging beyond original size will reduce sharpness — always work with high-resolution originals for best results.' },
    { q: 'Can I resize HEIC photos from my iPhone?',
      a: 'Yes — upload your HEIC files directly. They\'ll be decoded in your browser and resized to your chosen dimensions. Perfect for resizing iPhone photos before sharing on Windows or Android devices.' },
    { q: 'What does locking the aspect ratio mean?',
      a: 'Aspect ratio is the proportional relationship between width and height. Locking it means when you change one dimension the other adjusts automatically — keeping your image proportional and preventing distortion.' },
    { q: 'What size should I use for Instagram?',
      a: '1080×1080 for square posts, 1080×1350 for portrait posts, and 1080×1920 for Stories and Reels. Use the Instagram presets tab for one-click sizing.' },
    { q: 'Can I resize multiple images at once?',
      a: 'Yes — upload up to 20 images at once and they\'ll all be resized to the same dimensions. Download individually or use Download All as ZIP.' },
    { q: 'Can I resize by percentage instead of pixels?',
      a: 'Yes — switch to the Percentage tab and use the slider or quick buttons (25%, 50%, 75%, etc.) to scale images by any percentage from 1% to 300%.' },
    { q: 'Are my images uploaded to a server when I resize them?',
      a: 'No — all resizing happens entirely in your browser using the HTML5 Canvas API. Your images never leave your device and are never stored on any server.' },
    { q: 'What image formats can I resize?',
      a: 'JPG, PNG, WebP, and HEIC are all supported as input. You can download the resized result as JPG, PNG, or WebP — whichever suits your needs best.' },
    { q: 'Can I resize a PNG without losing its transparent background?',
      a: 'Yes — select PNG as the output format before downloading. The resizer preserves transparency in PNG files so your transparent background stays intact after resizing.' },
  ];
  ...
  return (
    <>
      ...
      <Script id="howto-schema-resizer" type="application/ld+json" strategy="afterInteractive"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            description: 'Resize JPG, PNG, WebP and HEIC images to any pixel size or percentage instantly in your browser — free, private, no uploads.',
            url: 'https://convertlinx.com/image-resizer',
            totalTime: 'PT30S',
            estimatedCost: { '@type': 'MonetaryAmount', value: '0', currency: 'USD' },
            supply: [{ '@type': 'HowToSupply', name: 'JPG, PNG, WebP, or HEIC image file' }],
            tool:  [{ '@type': 'HowToTool',   name: 'ConvertLinx Image Resizer'          }],
            step: [
              { '@type': 'HowToStep', name: 'Upload Images',    text: 'Upload up to 20 JPG, PNG, WebP, or HEIC images by dragging and dropping or clicking to browse.' },
              { '@type': 'HowToStep', name: 'Set Dimensions',   text: 'Choose Pixels for exact size, Percentage to scale proportionally, or pick a social media Preset.' },
              { '@type': 'HowToStep', name: 'Download Results', text: 'Click Resize and download each file individually or all at once as a ZIP.' },
            ],
          }),
        }}
      />
      ...
      <Script id="breadcrumb-schema-resizer" type="application/ld+json" strategy="afterInteractive"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            '@context': 'https://schema.org', '@type': 'BreadcrumbList',
            itemListElement: [
              { '@type': 'ListItem', position: 1, name: 'Home',          item: 'https://convertlinx.com/'               },
              { '@type': 'ListItem', position: 2, name: 'Image Resizer', item: 'https://convertlinx.com/image-resizer'  },
            ],
          }),
        }}
      />
      ...
      {/* ── SCHEMA: FAQPage ── */}
      <Script id="faq-schema-resizer" type="application/ld+json" strategy="afterInteractive"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
      ...
      <main className="ir-page">
        ...
        <section className="ir-hero-bg py-16 px-6 text-center">
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            ...
            <p className="text-base md:text-lg max-w-2xl mx-auto leading-relaxed" style={{ color: '#6B7280' }}>
              Resize JPG, PNG, WebP and HEIC images to exact pixel dimensions, by percentage, or
              using social media presets for Instagram, YouTube, Facebook, Twitter/X and more —
              instantly in your browser. 100% private — nothing uploaded, no signup required.
            </p>
            <div className="flex flex-wrap justify-center gap-2 mt-5">
              {['No upload needed', 'JPG · PNG · WebP · HEIC', 'Batch up to 20 images', 'Social media presets', '100% private'].map((t, i) => (
            ...
            <div className="ir-tool-card rounded-3xl p-6 md:p-8">
              ...
              {files.length > 0 && (
                <>
                    ...
                    {files.map((item, idx) => (
                      <div key={idx} className={`ir-file-row ${item.status}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold truncate" style={{ color: '#1a1a2e' }}>{item.file.name}</p>
                          <p className="text-xs" style={{ color: '#9CA3AF' }}>
                            {item.w}×{item.h}px · {fmtSize(item.file.size)}
                            {item.status === 'done' && (
                              <span style={{ color: '#10B981' }}> → {item.resultW}×{item.resultH}px · {fmtSize(item.resultSize)}</span>
                            )}
                          </p>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {item.status === 'done' && <CheckCircle className="w-4 h-4" style={{ color: '#10B981' }} />}
                  ...
                  {/* Resize Button */}
                  <button onClick={handleResizeAll} disabled={loading || !canResize}
        ...
        <section className="ir-alt-section py-16 px-6">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {[
                { icon: <Lock className="w-6 h-6" />,     color: '#F43F5E', bg: 'rgba(244,63,94,0.08)',   title: '100% Private',          desc: 'Images are resized entirely in your browser. Nothing is ever uploaded to any server — your files stay on your device.' },
                { icon: <Maximize2 className="w-6 h-6" />, color: '#8B5CF6', bg: 'rgba(139,92,246,0.08)', title: 'Aspect Ratio Lock',      desc: 'Enter width or height and the other dimension updates automatically — no distortion ever, even for batch resizing.' },
                { icon: <ImageIcon className="w-6 h-6" />, color: '#10B981', bg: 'rgba(16,185,129,0.08)', title: 'Social Media Presets',   desc: 'One-click presets for Instagram, YouTube, Facebook, Twitter/X and LinkedIn — perfectly sized every time, no guesswork.' },
              ].map((b, i) => (
        ...
        <section className="ir-main-section py-16 px-6">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                { num: '1', title: 'Upload Images',    desc: 'Drag & drop or click to upload — single image or up to 20 at once. JPG, PNG, WebP and HEIC supported.' },
                { num: '2', title: 'Set Dimensions',   desc: 'Use Pixels for exact size, Percentage to scale proportionally, or Presets for social media platforms.' },
                { num: '3', title: 'Download Results', desc: 'Click Resize and download each file individually or all at once as a ZIP — no account needed.' },
              ].map((s, i) => (
          ...
          <div className="max-w-3xl mx-auto space-y-8" style={{ color: '#6B7280' }}>
            ...
            <div>
              <h2 className="text-2xl font-bold mb-4" style={{ color: '#1a1a2e' }}>
                Why You Need a Free Online Image Resizer
              </h2>
              <p className="leading-7 text-sm">
                Every platform expects images in different sizes. Instagram needs square or portrait crops,
                YouTube thumbnails need 1280×720 pixels, Facebook cover photos have their own exact
                dimensions, and email newsletters need lightweight images that load fast. Resizing
                manually to the right pixel size saves time, prevents blurry or stretched photos,
                and keeps your file sizes small for faster uploads.
              </p>
              <p className="leading-7 text-sm mt-3">
                This free online image resizer works directly in your browser — no software to install,
                no file uploads, and no account needed. Enter exact pixel dimensions, scale by
                percentage, or pick a one-click social media preset for perfect results every time.
              </p>
            </div>

            <div>
              <h3 className="font-bold text-lg mb-3" style={{ color: '#1a1a2e' }}>
                Resize Images to the Right Size for Every Platform
              </h3>
              <p className="leading-7 text-sm">
                Different platforms require very different image sizes. Instagram square posts need
                1080×1080px, YouTube thumbnails require 1280×720px, Twitter/X post images look best
                at 1600×900px, and LinkedIn cover photos are 1584×396px. Instead of guessing and
                reuploading, use the built-in presets to hit the exact dimensions in one click — then
                download and upload directly with confidence.
              </p>
            </div>
            ...
            <div className="ir-seo-box rounded-2xl p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  'Instagram post blurry — wrong pixel dimensions',
                  'YouTube thumbnail not fitting — need exact 1280×720px',
                  'Profile photo distorted — need square resize without crop',
                  'Image file too large — need to scale down by percentage',
                  'Batch resize 20 product photos to same size',
                  'PNG transparency lost — need lossless resize',
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-2.5 text-sm">
                    <span className="ir-feature-dot w-1.5 h-1.5 rounded-full shrink-0" />
                    <span>{item}</span>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  'Social media creators — resize posts & thumbnails for every platform',
                  'Website owners — optimize images for faster page load speed',
                  'Designers — batch resize assets for multiple breakpoints',
                  'Online sellers — resize product photos to exact platform specs',
                  'Bloggers — resize featured images before WordPress upload',
                  'Everyone — quickly change image dimensions without Photoshop',
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-2 text-sm">
                    <span className="font-bold mt-0.5" style={{ color: '#F43F5E' }}>→</span>
                    <span>{item}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="ir-seo-box rounded-2xl p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  'Resize images in pixels or percentage',
                  'Batch resize up to 20 images at once',
                  'Social media presets — Instagram, YouTube, Facebook, Twitter/X, LinkedIn',
            ...
            <div>
              <h3 className="font-bold text-lg mb-4" style={{ color: '#1a1a2e' }}>
                Best Uses for Image Resizing
              </h3>
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  'Resize photos to 1080×1080 for Instagram square posts',
                  'Resize images to 1280×720 for YouTube thumbnails',
                  'Resize to 1080×1920 for Instagram Stories and TikTok',
                  'Scale down large RAW or DSLR photos by percentage',
                  'Resize iPhone HEIC photos and download as JPG or PNG',
                  'Batch resize product images for Shopify or WooCommerce',
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-2.5 text-sm">
                    <span className="ir-feature-dot w-1.5 h-1.5 rounded-full shrink-0" />
                    <span>{item}</span>
              ...
              {faqs.map((faq, i) => (
                <details key={i} className="ir-faq-item rounded-xl p-5">
                  <summary className="flex items-center justify-between gap-4">
                    ...
                    <ChevronDown className="w-4 h-4 shrink-0" style={{ color: '#F43F5E' }} />
                  </summary>
        ...
        {/* ── RELATED TOOLS ── */}
        <hr className="ir-mid-divider" />
        <section className="ir-alt-section py-14 px-6">
          <div className="max-w-3xl mx-auto">
...
//                 "position": 2,
//                 "name": "Image Resizer",
//                 "item": "https://convertlinx.com/image-resizer"
//               }
//             ]
//           })
//         }}
...
//                 "acceptedAnswer": {
//                   "@type": "Answer",
//                   "text": "Yes — the Convertlinx Image Resizer is completely free and works directly in your browser."
//                 }
//               },
//               {
//                 "@type": "Question",
...
//                 "acceptedAnswer": {
//                   "@type": "Answer",
//                   "text": "JPG, PNG, WebP and HEIC images are supported depending on your browser."
//                 }
//               },
//               {
//                 "@type": "Question",
...
//                 "acceptedAnswer": {
//                   "@type": "Answer",
//                   "text": "Yes — you can upload and resize up to 20 images in a batch."
//                 }
//               },
//               {
//                 "@type": "Question",
...
//                 "acceptedAnswer": {
//                   "@type": "Answer",
//                   "text": "No — resizing happens directly in your browser so your images never leave your device."
//                 }
//               }
//             ]
//           })
...
//             <div>
//               <h2 className="text-2xl font-bold mb-4" style={{ color: '#1a1a2e' }}>
//                 Free Image Resizer — ConvertLinx
//               </h2>

//               <p className="leading-7 text-sm">
//                 The <strong>Convertlinx Image Resizer</strong> lets you
//                 <strong>resize images online</strong> instantly without uploading
//                 files to any server.

//                 You can change <strong>image dimensions in pixels</strong>,
//                 resize by <strong>percentage</strong>, or use
//                 <strong>social media presets</strong> for Instagram,
//                 YouTube, Facebook, and Twitter.

//                 This <strong>free image resizing tool</strong> supports
//                 JPG, PNG, WebP, and HEIC images and works directly
...
//               <div className="grid sm:grid-cols-2 gap-3">
//                 {[
//                   'Social media creators — resize posts & thumbnails',
//                   'Website owners — optimize images for web',
//                   'Designers — quick resizing for multiple platforms',
//                   'Students — resize photos for projects',
//                   'Everyone — quickly change image dimensions',
//                 ].map((item, i) => (
//                   <div key={i} className="flex items-start gap-2 text-sm">
//                     <span className="font-bold mt-0.5" style={{ color: '#F43F5E' }}>→</span>
//                     <span>{item}</span>
...
//                 {[
//                   'Resize images in pixels or percentage',
//                   'Batch resize up to 20 images',
//                   'Social media presets (Instagram, YouTube, Facebook)',
//                   'Lock aspect ratio to prevent distortion',
//                   'Export as JPG, PNG or WebP',
//                   'Quality slider for file size control',
